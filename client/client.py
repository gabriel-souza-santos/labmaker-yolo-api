    ...
    for attempt in range(max_retries):
        try:
            ...
            if r.status_code == 200:
                data = r.json()
                print(
                    f"[OK] API disponível | modelo: {data['model_name']}"
                )
                return True
...
def run_single_inference(image_path: Path, confidence: float = 0.25):
    ...
    print(
        f" Resolução           : {data['image_width']}x{data['image_height']} px"
    )
    print(f" Detecções ({len(data['detections'])}):")
    for det in data["detections"]:
        print(
            f"   • {det['label']:20s} conf={det['confidence']:.2f}"
        )
...
def run_batch_inference(image_paths: list, confidence: float = 0.25):
    ...
    for i, r in enumerate(data["results"]):
        print(
            f" Imagem {i + 1}: {len(r['detections'])} detecções em {r['inference_ms']} ms"
        )


if __name__ == "__main__":
    wait_for_api()
    images = sorted(IMAGES_DIR.glob("*.jpg")) + sorted(
        IMAGES_DIR.glob("*.png")
    )
